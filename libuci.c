...
__private char uci_confdir[MAX_PATH] = "";
__private char uci_savedir[MAX_PATH] = "";

static bool initialize_dirs(void)
{
	if (uci_confdir[0] != '\0')
		return true;

	LPITEMIDLIST pidl;
	if (SHGetSpecialFolderLocation(NULL, CSIDL_PROFILE, &pidl) != NOERROR)
		return false;
	...
	strcpy(uci_confdir, path);
	strcat(uci_confdir, UCI_CONFDIR);

	strcpy(uci_savedir, path);
	strcat(uci_savedir, UCI_SAVEDIR);
